...
public class ImportLayoutCommand extends AbstractCommand {
    @Override
    public void execute() {
        ResourceManager manager = ResourceManager.get(ImportLayoutCommand.class);
        Path layoutsFolder = manager.getHomePath().resolve("layouts");
        File layoutFolder = layoutsFolder.toFile();
        ...
        chooser.setDialogTitle("Layout Import");
        ...
        try {
            copyFileUsingStream(file, layoutsFolder.resolve(file.getName()).toFile());
        } catch (IOException e) {
