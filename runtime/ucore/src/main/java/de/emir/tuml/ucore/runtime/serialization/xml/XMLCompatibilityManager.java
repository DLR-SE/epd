 ...
 * Therefore, you can register an instance of de.emir.tuml.ucore.runtime.serialization.xml.XMLSerializer.IXMLReadListener
 * that will be notified if a new object instance has been created.
 * @author sschweigert
 *
 */
public class XMLCompatibilityManager implements IExtensionPoint, IXMLReadListener {
	...
	public interface IXMLPostProcessor extends IXMLReadListener {
		/**
         * Will be asked for every read object, if a postprocessing is required. If the method returns true, the
         * onObjectRead method is called
         */
		public boolean requiresPostProcessing(UObject obj, final Node xmlNode, XMLReader reader);
	}
	
	private final ArrayList<IXMLPostProcessor> 		mDelegates = new ArrayList<>();
	
	public XMLCompatibilityManager() {
	}

	/**
	 * Register a new Compatibility handler that shall be notified, if a new instance that is of the same type as the
     * specified clazz has been created
	 * @param handler
	 */
