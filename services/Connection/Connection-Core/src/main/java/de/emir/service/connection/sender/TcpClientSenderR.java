...
import de.emir.service.connection.receiver.TcpServerReceiverWB;
...
public class TcpClientSenderR implements Sender, Receiver {
	 ...
	 *            Destination port.
	 */
	public TcpClientSenderR(final Listener listener, String adr, int p) {
	 ...
	 */
	public TcpClientSenderR(final SenderListener listener, String adr, int p, String localadr) {
	...
	public void receive() {
		thread = new Thread() {
			...
			public void run() {
				try {
					InputStream is = TcpClientSenderR.this.socket.getInputStream();
					BufferedReader reader = new BufferedReader(new InputStreamReader(is));
					while (run) {
						String line = null;
						while (reader != null && (line = reader.readLine()) != null) {
//							lastTimestamp = Calendar.getInstance().getTimeInMillis();
							rlistener.onReceived(TcpClientSenderR.this, line.getBytes());
						}
						Thread.sleep(0);
					}
//					reader.close();
