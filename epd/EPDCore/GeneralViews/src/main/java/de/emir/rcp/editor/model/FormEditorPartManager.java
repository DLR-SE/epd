...
public class FormEditorPartManager implements IExtensionPoint {
     ...
     *
     * @param cl classifier that should no longer be displayed, using the editor part
     * @param part editor part provider that should be removed
     * @note if the part provider has been registered for other classifier, they will still be returned by
     * provideEditorPart()
     */
    ...
    /**
     * checks the registered FormEditorPartProvider and returns one if one has been registered for this class
     *
     * @param cl classifier that shall be displayed
     * @param ptr pointer to be represented by this editor part (maybe null)
     * @param returnDefault if set to true, the default form editor is returned, that is a PropertySheet panel,
     * displaying the same informations as the de.emir.rcp.views.properties.PropertyView
     * @return a form provider that can generate a form for this type of object or null, if no provider has been
     * registered
     * @note if more than one editor part has been registered for this classifier, the first one is returned
     */
    ...
    /**
     * checks the registered FormEditorPartProvider and returns all if any has been registered for this class
     *
     * @param cl classifier that shall be displayed
     * @param ptr pointer to be represented by this editor part (maybe null)
     * @param returnDefault if set to true, the default form editor is returned, that is a PropertySheet panel,
     * displaying the same informations as the de.emir.rcp.views.properties.PropertyView
     * @return a list of form providers that can generate a form for this type of object or an empty list, if no
     * provider has been registered
     */
    public List<IFormEditorPart> provideAllEditorParts(UClassifier cl, Pointer ptr, boolean returnDefault) {
        List<IFormEditorPart> result = new ArrayList<>();
        if (mProvider.containsKey(cl)) {
            if (ptr == null) {
                result.addAll(mProvider.get(cl));
            }
            for (IFormEditorPart fep : mProvider.get(cl)) {
                if (fep.supports(ptr)) {
                    result.add(fep);
     ...
     * provider has been registered
     */
    public List<IFormEditorPart> provideAllEditorParts(UClassifier cl, boolean returnDefault) {
        return provideAllEditorParts(cl, null, returnDefault);
    }
}
